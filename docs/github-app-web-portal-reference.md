...
결과가 아니라 delivery 경계 확인용 예시다. 처음 실행하는 사용자는 준비 조건과 화면 확인 순서를
정리한 [로컬 no-network 사용 가이드](github-app-web-portal-local-guide.md)를 먼저 따른다.
...
7. 모바일·PC browser에서 비개발자 최소 2명이 repository 선택, 변경 이해, 승인 취소·오류 복구,
