...
- 법률·개인정보 검토 책임자, evidence와 Production 전 결정 기한
- 데이터 category별 retention·파기·legal hold 정책 책임자, evidence와 Production 전 결정 기한
- 단일·다중 instance topology, rate-limit key·shared store/gateway/WAF 전략, fail-open/closed와 책임자
...
함께 적용한다. 초기 답변이 없으면 [Production readiness 초기 template](templates/production-readiness.json)을
project의 `docs/production-readiness.json`으로 검토·materialize하고 `TBD`·owner·결정 기한을 보존한다.
...
- 관련 요구사항: REQ-007, REQ-020, REQ-021, REQ-037, REQ-040, REQ-045, REQ-046
