...
현재 이 단계는 범용 downstream installer가 아니다.
...
- `security-tools`는 현재 **upstream clone의 `.tools/`**에 검증 도구를 설치한다. downstream에
  materialize하거나 설치하지 않는다. 다운로드 전에 exact version·checksum·network 승인을 확인한다.
