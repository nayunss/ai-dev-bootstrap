...
상태: shared adoption core·GitHub Actions P0 reference 구현
검토일: 2026-07-19
관련 요구사항: `REQ-047`
...
GUI·CLI·GitHub Actions가 각각 파일 적용 로직을 구현하지 않는다.
...
2026-07-19 GitHub Actions P0 reference는 `workflow_dispatch` preview와 protected environment
apply를 분리하고, exact plan SHA-256 재검증 뒤 새 branch와 PR만 만들도록 구성했다. Web·GUI·CLI
plan parity와 dirty checkout·stale plan·unreviewed release 차단은 deterministic PASS다. 실제
downstream hosted run과 사람 승인은 `NOT-RUN`이며, GitHub App Portal은 해당 pilot PASS 뒤에만
진행한다. 상세 중단 조건은 [GitHub 기반 Web Adoption Delivery](web-adoption-delivery-review.md)를
따른다.
