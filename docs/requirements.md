...
- Portal App은 선택한 repository만 설치 대상으로 허용하고 Metadata read, Contents·Pull Requests
  write 밖의 권한을 요청하지 않는다. Installation token은 ephemeral checkout callback, user access
  token은 사용자 대신 PR을 만드는 delivery callback 안에서만 사용하며 저장·log·고정 형식 가정을
  금지한다.
- OAuth callback state는 한 번만 사용하고 actor·installation account·repository와 묶는다. Webhook은
  raw body HMAC-SHA256을 검증한 뒤 delivery ID replay·중복을 차단하며 suspension·uninstall·user
