...
| REQ-047 | 부분 구현 | deterministic PASS | GUI·CLI·web 공통 adoption core, macOS 개발 shell·resilience·unsigned package provenance와 GitHub Actions preview→승인→PR P0 reference 구현. Clean·upgrade·rollback·변조·충돌·drift·부분 실패·crash·권한 fixture PASS. Apple credential 미보유로 desktop signing은 BLOCKED, 실제 downstream web pilot·GitHub App Portal·screen reader·비개발자 Eval은 NOT-RUN |
...
| 비개발자 도입 | REQ-047 | GUI·CLI·web 공통 core, desktop reference와 GitHub Actions read-only preview·protected apply→PR P0 구현, installer·Portal 미발행 | Core surface parity, clean checkout·plan binding·least privilege·stage-only·no-auto-merge deterministic PASS. Desktop credential BLOCKED, 실제 web pilot·Portal·사람 사용성 NOT-RUN | `REQ-047-gui-installer-delivery`, `REQ-047-github-app-web-portal` | [GUI 설치 자산·배포 준비](gui-installation-distribution-review.md), [Web Adoption](web-adoption-delivery-review.md) |
...
- GUI와 전문가·CI용 CLI는 서로 다른 설치 로직을 구현하지 않고 release pin, checksum·서명 검증,
  ...
  사용해야 한다.
- 지원 desktop credential이 없는 경우 GitHub 기반 web adoption을 별도 경로로 제공할 수 있다.
...
- 설치 자산 게시·검증 gate는 `docs/gui-installation-distribution-review.md`, desktop UX·공통 core의
  단계적 구현과 완료 기준은 `docs/one-click-project-adoption-review.md`, GitHub Actions와 향후
  GitHub App Portal 경계는 `docs/web-adoption-delivery-review.md`를 따른다.
