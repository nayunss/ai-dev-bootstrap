...
| 금지 | 사용자 전역 AI·Git·shell 설정 강제 변경, 승인되지 않은 plugin/network 자동 설치, 보안 gate 개인 해제, 비밀 공유 | 저장하지 않음 | 승인으로 허용 범위를 넓힐 수 없는 기본 금지 또는 별도 고위험 계약 | bootstrap·GUI·CLI 모두 fail-closed |
...
REQ-047 GUI와 CLI는 적용 항목을 다음처럼 구분해 표시한다.
...
- GUI와 CLI가 같은 분류·plan·lock 결과를 생성해야 한다.
