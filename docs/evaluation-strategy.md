...
| Dependency 승인 | 미승인 direct version·lockfile-only 변경 거부, 정확한 승인 범위 허용, 만료·불일치 거부 |
| 라이선스·출처 | dependency license, source snippet match, scanner outage fail-closed, suppression 만료 |
| API 계약 | contract syntax·drift, breaking change, undocumented endpoint, production docs exposure |
...
Skill 변경은 일반적인 prompt 비교보다 강한 격리를 적용한다. target model·harness·tool과 evaluator를
고정하고 train trajectory로 bounded patch를 제안한 뒤 selection에서 여러 trial과 최소 개선폭을
판정한다. test는 최종 1회 판정 전 잠그며 optimizer와 대상 agent가 grader·fixture·expected
outcome을 수정하지 못하게 한다.

동률 또는 필수 security·permission·correctness·regression gate 실패는 거절한다. 거절 patch와
이유는 다음 제안의 negative evidence로 기록하되 민감한 trajectory 원문은 보관하지 않는다. 선택된
후보도 사람 승인과 test를 통과해야 release할 수 있다. 상세 계약은
...
- 관련 요구사항: REQ-004, REQ-009–REQ-016, REQ-022–REQ-025, REQ-041
