...
2026-07-18 P0 reference fixture로 React+Vite 단일 frontend, Node+Express+PostgreSQL 단일 backend,
Next.js+PostgreSQL 단일 full-stack과 React+Vite/Node+Express+PostgreSQL workspace monorepo를
구현했다. Clean target materialization, 기존 동일 파일 보존 retrofit, 충돌 전면 차단과 생성 소유
파일 rollback을 검증하며 dependency install·DB migration·provider write·Production deploy는
`NOT_RUN`이다. 이는 실제 runtime·DB pilot 또는 P1 이상 stack 지원 완료를 뜻하지 않는다.
