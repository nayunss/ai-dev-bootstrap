...
상태: core·adapter·upgrade reference 구현, 실제 downstream 적용 별도 승인
갱신일: 2026-07-19
...
도입 절차는 [처음부터 끝까지 사용 가이드](bootstrap-user-guide.md)를 따른다.

## 현재 구현 상태

이 문서의 pin → materialize → enforce → upgrade 흐름은 공통 core와 deterministic fixture로
구현돼 있다. 실제 downstream release 선택·적용, dependency network, DB·provider write와
Production 배포는 project별 입력·승인·환경 증거가 필요한 별도 단계다.

| 항목 | 현재 상태 |
|---|---|
| release tag·archive checksum | v0.2.0~v0.2.8 pilot 발행 완료, v0.2.9 candidate 준비 |
...
| 비개발자 web surface | GitHub Actions P0 실제 delivery pilot PASS, GitHub App Portal local no-network reference PASS |

Reference PASS를 실제 dependency 설치, GitHub App 등록, provider write 또는 Production 지원
완료로 확대하지 않는다.
...
- 관련 요구사항: REQ-001–REQ-008, REQ-019, REQ-027, REQ-038, REQ-042, REQ-045–REQ-046
- 관련 문서: [권장 아키텍처](architecture.md),
  [유지보수와 도입 모델](adoption-and-maintenance-model.md),
  [처음부터 끝까지 사용 가이드](bootstrap-user-guide.md),
