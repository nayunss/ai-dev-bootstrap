...
상태: `DEFERRED / OUT-OF-SCOPE` (과거 검토·개발 증거 보존)
검토일: 2026-07-19
관련 요구사항: `REQ-047`
...
현재 공식적으로 설치할 수 있는 desktop GUI는 없으며 현 제품 범위에서 발행할 계획도 없다.
`v0.2.8-pilot`은 GUI·CLI가 공유하는 headless
...
- 기본 비개발자 경로: [GitHub 기반 Web Adoption Delivery](web-adoption-delivery-review.md)
...
| Publisher identity·credential | `UNAVAILABLE` | Apple Developer Program 미가입, Developer ID Application 인증서 미보유 |
| Code signing·notarization | `DEFERRED / OUT-OF-SCOPE` | Portal이 기본 surface로 결정되어 활성 backlog에서 제외 |
...
아래 목록은 이미 수행한 개발 증거와 향후 범위를 재개할 때 필요한 gate를 보존한 기록이다.
`REQ-047-gui-installer-delivery`와 GUI-05–GUI-07은 활성 작업이 아니다.
...
| 5 | `GUI-05-signing-notarization` | Publisher 법적 표시명, credential owner·보관·회전·폐기·감사 계약을 승인하고 OS별 code signing·notarization 또는 동등 검증을 수행한다. | 서명·notarization log, clean-machine OS 검증, credential 비노출 증거 | `DEFERRED / OUT-OF-SCOPE` |
| 6 | `GUI-06-usability-eval` | Terminal 비숙련 사용자가 신규·기존 프로젝트에서 설치, preview, 적용, 오류 이해, rollback과 uninstall을 수행하는 접근성·사용성 Eval을 진행한다. | 사전 정의 task·합격 기준, 참여자 결과, blocker 0, keyboard·screen reader PASS | `DEFERRED / OUT-OF-SCOPE` |
| 7 | `GUI-07-release-publish` | 모든 gate가 PASS한 OS asset만 exact release에 게시하고 README·release note·manifest의 지원 상태를 동기화한다. 게시 후 asset을 다시 내려받아 signature와 checksum을 재검증한다. | 승인된 release, 게시 asset digest, 재다운로드 PASS, migration·rollback note | `DEFERRED / OUT-OF-SCOPE` |
...
Credential 없이 진행하는 별도 GitHub 경로는
[GitHub 기반 Web Adoption Delivery](web-adoption-delivery-review.md)를 따른다.

## 완료 판정

현재 범위에서는 이 완료 판정을 실행하지 않는다. Desktop 범위가 별도 승인되어 재개되는 경우에만
