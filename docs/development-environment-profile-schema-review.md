...
| 산출물 | 역할 | 구현 상태 |
|---|---|---|
| `docs/development-environment.profile.yaml` | 프로젝트별 canonical machine-readable profile | reference materializer 구현 |
| `docs/development-environment.md` | 사람이 읽는 설명·결정 근거·운영 안내 | 기존 계약 |
| `docs/schemas/development-environment-profile.schema.json` | YAML parse 결과의 구조 계약 | reference 구현 |
| `scripts/validate-development-environment-profile.mjs` | 의미·drift·readiness validator | reference 구현 |
| profile fixture | single frontend/backend/full-stack/monorepo와 negative case | deterministic PASS |
...
- Portal·CLI에서 동일한 schema·validator·blocker 결과 사용
...
- Portal·CLI가 같은 validation core와 결과 schema를 사용
...
2026-07-18 P0 구현은 canonical serializer와 제한된 YAML parser, 구조·의미·repository·readiness
validator 및 synthetic fixture까지 완료했다. REQ-020·021 범위의 P1 initial/retrofit materializer도
질문·preview·명시 승인, 동일 파일 보존과 다른 기존 파일의 atomic 충돌 차단까지 구현했다.
Production 단계는 별도 gate가 필요하므로 항상 `PROFILE_PRODUCTION_SEPARATE_GATE`를 반환한다.
Schema migration·Markdown 참조 검사, 실제 downstream clean/retrofit과 release upgrade·rollback은
완료 범위가 아니다.
