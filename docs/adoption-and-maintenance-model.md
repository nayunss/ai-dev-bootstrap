...
REQ-001–REQ-046 설계 명세는 `DESIGN-BASELINE-2026-07-17`로 완료됐고 실제 공통 환경 구현 단계로
전환한다. 저장소의 기존 bootstrap·validator·fixture는 설계 누락과 호환성 문제를 발견한 reference
automation이며, 목표 환경 전체가 실제로 구현됐다는 표시는 아니다.

구현은 `docs/requirements.md`와 `docs/design-completion-audit.md`를 입력 계약으로 사용한다. 구현 중
pilot 결과를 요구사항·설계·Eval·HANDOFF에 계속 환류하고, 미검증 항목을 구현 완료나 지원 완료로
승격하지 않는다. pilot의 특정 stack은 검증 사례이며 사용자 선택 전 공통 기본값이 아니다.
...
| 전환 | 적용 방식 |
|---|---|
| 신규 full-stack | frontend·backend·shared contract와 공통 하네스를 한 계획으로 적용 |
| backend → full-stack | backend baseline을 보존하고 frontend 및 영향받는 공통 계층만 증분 적용 |
| frontend → full-stack | frontend baseline을 보존하고 backend·DB·migration 및 통합 경계만 증분 적용 |
| service 추가 | 새 service manifest·CI·deploy와 공통 inventory를 추가하고 기존 service drift 검사 |

증분 적용도 단순 파일 복사가 아니다. 저장소를 다시 탐색해 application inventory를 갱신하고 CodeSight,
EditorConfig, AI adapter, hook, security-check, CI와 HANDOFF가 새 경계를 포함하는지 확인한다. 기존 profile의
성공 기록은 새 application의 품질·보안·배포 검증을 대신하지 않는다.

Upstream은 `scripts/bootstrap preview <target>`에서 root·하위 manifest와 선언 여부를 읽기 전용으로
표시하고, `scripts/validate <target>`에서 `docs/development-environment.md`의 JSON inventory를 실제
manifest와 대조한다. 다중 application은 각 항목의 quality command, CI·deploy evidence와 hook 상태를
선언하고 공통 CodeSight·EditorConfig profile을 검증해야 한다. `hook: not-approved`는 누락을 숨기는
성공 상태가 아니라 설치 승인 대기 상태이며 HANDOFF에 남긴다.
...
- 관련 요구사항: REQ-001–REQ-008, REQ-019, REQ-023, REQ-025–REQ-027, REQ-045–REQ-046
- 관련 문서: [아키텍처](architecture.md), [하네스](harness.md),
  [공급망 보안](supply-chain-security.md), [Eval 전략](evaluation-strategy.md),
  [다중 참여자 Pilot 검증](distributed-pilot-testing-guide.md)
...
- multi-application 저장소의 dependency·quality 명령은 각 application root를 실제 cwd로 사용한다.
  root manifest가 Maven인 저장소에서 `pnpm --dir frontend`를 쓰면 Corepack이 root에서 floating pnpm을
  찾을 수 있으므로 root fallback으로 대체하지 않는다.
...
| nested application cwd | application root의 exact packageManager만 사용하고 root fallback 금지 |
...
| CodeSight 누적 timestamp | `wiki/log.md`를 결정론적 상태로 정규화하고 Git diff로 stale 판정 |
| 3-service PR 격리 | 동일 Preview environment의 frontend·backend·DB와 Production negative login 증거 요구 |
| application rollback | 실패 healthcheck revert와 DB migration·restore rollback을 별도 경계로 기록 |
| API contract drift | stack adapter를 승인하고 필수 operation·response·schema와 breaking fixture·docs exposure 검사 |
...
현재 완료된 pilot은 Next.js frontend 한 건과 Spring Boot→Next.js 증분 full-stack 한 건이다. 공통
환경의 stack 중립성을 주장하기 전에 다른 stack과 최초 full-stack 일괄 구성도 별도 downstream에서
검증한다.

### 최초 full-stack reference materializer

`docs/schemas/fullstack-materializer.schema.json`과 `scripts/materialize-fullstack.mjs`는 stack을
고정하지 않고 frontend·backend·shared·migration root와 file hash를 profile로 입력받는다. preview는
파일을 쓰지 않으며 apply·rollback은 `--approve`를 요구한다.

apply는 기존 파일 충돌을 쓰기 전에 전부 검사하고, 생성 파일·lock·rollback record를 하나의 file
transaction으로 기록한다. 중간 write가 실패하면 적용 전 snapshot으로 복구한다. rollback은
materializer가 생성했고 hash가 유지된 파일만 제거하며 적용 전부터 동일했던 파일은 보존한다.

database migration은 up·rollback artifact pair와 hash만 materialize한다. lock과 결과에는
`databaseExecution: NOT-RUN`을 기록하며 SQL 실행 기능이나 DB credential 입력을 제공하지 않는다.
실제 migration, backup·restore와 rollback 실행은 별도 preview·승인·격리 환경 검증 대상이다.

> 참고: pilot은 제품 기능의 완성도를 평가하는 프로젝트가 아니다. 프론트엔드 Todo 사례처럼 선택한
> 스택의 bootstrap·보안·품질·CI·배포 경로를 빠르게 확인할 수 있는 최소 기능만 구현한다. 기능 범위가
> 커져 하네스 검증보다 제품 개발이 중심이 되지 않도록 한다.
...
- 검증: exact runtime·build tool, dependency lock·verification, secret 비접근, DB least privilege,
