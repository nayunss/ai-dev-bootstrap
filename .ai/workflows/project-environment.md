...
REQ-040·041·042의 프로젝트별 결정 profile을 함께 준비할 때는 초기 project와 기존 project 모두
`scripts/bootstrap onboarding <target>`을 사용한다. 명령은 누락된
`docs/production-readiness.json`, `docs/skill-evolution-trial.json`,
`docs/upstream-adoption.json`만 생성하고 기존 파일을 보존한다. 출력된 질문은 해당 project owner가
답하며 미응답 `TBD`·`pending`은 Production, 유료 model 호출, release 적용 승인이 아니다.

REQ-020·021의 canonical 개발환경 profile은 검토된 JSON 입력을
`scripts/bootstrap profile TARGET PROFILE.json`으로 먼저 preview한다. 입력은 팀 필수·project 필수·
개인 선택·사전 심사·금지 경계를 포함해야 하며, 개인 선택이 팀 gate를 약화하거나 금지 항목과
중복되면 차단한다. 확인 후 같은 명령에 `--approve`를 추가하면 profile이 없는 target에만
`docs/development-environment.profile.yaml`을 생성한다. 기존 파일이 byte-identical이면 보존하고
다르면 자동 병합·덮어쓰기 없이 `PROFILE_EXISTING_CONFLICT`로 중단한다.
