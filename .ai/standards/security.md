...
- scripts-off install 통과는 lifecycle script 실행 승인이 아니다.
- pnpm 11 이상은 `strictDepBuilds`를 끄지 않고 `allowBuilds`에 package와 정확한 version별 `true` 또는
  `false` 결정을 기록한다.
- `dangerouslyAllowAllBuilds`, wildcard와 version 없는 matcher는 금지한다.
- 허용 전 script source, publisher, integrity, network·filesystem 영향과 production 필요성을 심사한다.
- provider-equivalent clean install은 비밀 없는 격리 환경에서 실행하고 예상 밖 script가 발견되면
  default-deny로 중단한다.
