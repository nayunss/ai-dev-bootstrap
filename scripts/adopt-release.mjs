...
import { runCliAdoption } from "./release-adoption-surfaces.mjs";
...
function main() {
  ...
  if (!["preview", "apply", "validate", "upgrade", "rollback"].includes(mode) || !target || !manifestPath) {
    process.stderr.write("Usage: adopt-release.mjs <preview|apply|validate|upgrade|rollback> TARGET MANIFEST.json [--surface=cli] [--approve]\n");
    process.exit(2);
  }
  const manifest = JSON.parse(readFileSync(resolve(manifestPath), "utf8"));
  const surface = flags.find((flag) => flag.startsWith("--surface="))?.slice("--surface=".length) ?? "cli";
  if (surface !== "cli") {
    process.stderr.write("Surface must be cli.\n");
    process.exit(2);
  }
  const result = runCliAdoption(mode, manifest, process.cwd(), target, {
