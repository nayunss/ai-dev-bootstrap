...
경로는 향후 GitHub App Web Portal이며, 그 전 단계로 Actions P0를 검증합니다. `curl | sh`,
...
사람이 승인한 exact plan만 새 branch와 PR로 제안합니다. 실제 downstream pilot은 아직
`NOT-RUN`이므로 production 지원 경로로 표시하지 않습니다.
