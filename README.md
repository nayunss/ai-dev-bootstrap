...
보안 경계, 승인된 도구 버전과 검증 절차로 개발하도록 묶어 주는 프로젝트 로컬
**AI 개발 하네스**입니다. 신규 프로젝트와 운영 중인 기존 프로젝트를 모두 지원하며 개인 전역 설정을
강제로 덮어쓰지 않습니다.

> 최신 발행·검증 기준: [`v0.2.9-pilot`](docs/releases/v0.2.9-pilot.md)
>
> 처음 사용하는 경우: [처음부터 끝까지 사용 가이드](docs/bootstrap-user-guide.md)

## 이 프로젝트의 분류

이 저장소는 하나의 앱·라이브러리·스킬이 아니라, 여러 AI 코딩 도구가 같은 규칙·보안·검증
아래에서 개발하도록 묶어 주는 **AI 개발 하네스**입니다. "부트스트랩"은 이 하네스를
프로젝트에 처음 들여오는 *도입 방식*을 가리키는 말이며, 정체 자체를 가리키는 말은 아닙니다.

| 흔한 오해 | 실제 |
|---|---|
| 템플릿·보일러플레이트 | ✗ 한 번 복사하고 끝이 아니라, upstream↔downstream이 release·checksum으로 계속 연결되고 `validate`와 보안 게이트가 반복 실행됩니다. |
| 에이전트·스킬 | ✗ 스스로 도는 실행체가 아니라, AI 도구(Codex·Claude Code·GitHub Copilot)가 지켜야 할 규칙과 검증을 담은 *실행 계약*입니다. |
| 부트스트랩 | △ 맞지만 "어떻게 시작하나"만 가리킵니다. 시작 이후에도 계속 집행되는 규칙·게이트가 본질입니다. |
| 하네스 | ◎ 도구 중립 코어 + 도구별 어댑터 + 자동 검증을 하나로 묶은 저장소 수준 실행 계약. 그래서 패키지 이름도 `common-ai-development-harness`입니다. |

정의와 구성 원칙은 [권장 아키텍처](docs/architecture.md)와 [하네스 구성](docs/harness.md)을 따릅니다.
...
```text
이 폴더는 AI 개발 하네스(ai-dev-bootstrap) 저장소를 clone했거나 공식 release ZIP을 푼 루트입니다.
...
전체 절차는 [처음부터 끝까지 사용 가이드](docs/bootstrap-user-guide.md)를 따르세요.
